 ...
 * {@link java.util.concurrent.ConcurrentHashMap} with better scaling properties
 * and generally lower costs to mutate the Map. It provides identical
 * correctness properties as ConcurrentHashMap. All operations are non-blocking
 * and multi-thread safe, including all update operations.
 * {@link NonBlockingHashMap} scales substantially better than
 * {@link java.util.concurrent.ConcurrentHashMap} for high update rates, even
 * with a large concurrency factor. Scaling is linear up to 768 CPUs on a
 * 768-CPU Azul box, even with 100% updates or 100% reads or any fraction
 ...
 * 
 * This class obeys the same functional specification as
 * {@link java.util.Hashtable}, and includes versions of methods corresponding
 * to each method of <tt>Hashtable</tt>. However, even though all operations are
 ...
 * 
 * <p>
 * Operations (including <tt>put</tt>) generally do not block, so may overlap
 * with other update operations (including other <tt>puts</tt> and
 * <tt>removes</tt>). Retrievals reflect the results of the most recently
 * <em>completed</em> update operations holding upon their onset. For aggregate
 * operations such as <tt>putAll</tt>, concurrent retrievals may reflect
 ...
 * 
 * <p>
 * Very full tables, or tables with high reprobe rates may trigger an internal
 * resize operation to move into a larger table. Resizing is not terribly
 * expensive, but it is not free either; during resize operations table
 * throughput may drop somewhat. All threads that visit the table during a
 * resize will 'help' the resizing but will still be allowed to complete their
 * operation before the resize is finished (i.e., a simple 'get' operation on a
 * million-entry table undergoing resizing will not need to block until the
 * entire million entries are copied).
 * 
 * <p>
 ...
 * 
 * @version 1.1.2
 * @author Prashant Deva - moved hash() function out of get_impl() so it is not
...
public class NonBlockingHashMap<TypeK, TypeV> extends AbstractMap<TypeK, TypeV>
	...
	private static final int REPROBE_LIMIT = 10; // Too many reprobes then force
													// a table-resize
	...
	// --- Adding a 'prime' bit onto Values via wrapping with a junk wrapper
	// class
	// 一个Object的包装类
	...
	//Slot0和Slot1存储特定数据，其他的key-value全部存储在kvs中，每个key-value占用2个位置
	//（Slots {2,3}, {4,5}, etc hold {Key,Value} pairs）
	//为啥_kvs不是volatile?
	...
	//Tombstone表示一个key已经被删除，这因为这里采用了开放地址法处理碰撞
	...
	private static final boolean CAS_key(Object[] kvs, int idx, Object old,
			Object key) {
		return _unsafe.compareAndSwapObject(kvs, rawIndex(kvs, (idx << 1) + 2),
				old, key);
	}

	private static final boolean CAS_val(Object[] kvs, int idx, Object old,
			Object val) {
		return _unsafe.compareAndSwapObject(kvs, rawIndex(kvs, (idx << 1) + 3),
				old, val);
	}
	 ...
	 * 
	 * @return the count of reprobes since the last call to {@link #reprobes} or
	 ...
	 * 
	 * @return <tt>true</tt> if the key is in the table using the
	 *         <tt>equals</tt> method
	 ...
	 * value. <em>Note</em>: This method requires a full internal traversal of
	 * the hash table and is much slower than {@link #containsKey}.
	 * 
	 * @param val
	...
	public Object clone() {
		try {
			...
			NonBlockingHashMap<TypeK, TypeV> t = (NonBlockingHashMap<TypeK, TypeV>) super
					.clone();
	...
	// 'equals' v-call.
	private static boolean keyeq(Object K, Object key, int[] hashes, int hash,
			int fullhash) {
		return K == key || // Either keys match exactly OR
				...
				// Do not call the users' "equals()" call with a Tombstone, as
				// this can
				// surprise poorly written "equals()" calls that throw
				// exceptions
				// instead of simply returning false.
						K != TOMBSTONE && // Do not call users' equals call with
											// a Tombstone
	...
	private static final Object get_impl(final NonBlockingHashMap topmap,
			final Object[] kvs, final Object key, final int fullhash) {
		final int len = len(kvs); // Count of key/value pairs, reads kvs.length
		final CHM chm = chm(kvs); // The CHM, for a volatile read below; reads
									// slot 0 of kvs
		final int[] hashes = hashes(kvs); // The memoized hashes; reads slot 1
											// of kvs
		...
		while (true) {
			...
			final Object K = key(kvs, idx); // Get key before volatile read,
											// could be null
			final Object V = val(kvs, idx); // Get value before volatile read,
											// could be null or Tombstone or
											// Prime
			if (K == null)
				return null; // A clear miss  如果第一个hash的位置为空，则说明key在hashtable中不存在

			// We need a volatile-read here to preserve happens-before semantics
			// on
			// newly inserted Keys. If the Key body was written just before
			// inserting
			// into the table， a Key-compare here might read the uninitalized Key
			// body.
			// Annoyingly this means we have to volatile-read before EACH key
			// compare.
			// .
			// We also need a volatile-read between reading a newly inserted
			// Value
			...
			// read covers both.
			final Object[] newkvs = chm._newkvs; // VOLATILE READ before key
													// compare
			...
			if (keyeq(K, key, hashes, idx, fullhash)) {
				...
				// Key hit - but slot is (possibly partially) copied to the new
				// table.
				// Finish the copy & retry in the new table.
				// 如果返回的V是Prime实例，则说明hashtable正在copy到新的table(rehash)
				// copy_slot_and_check应该返回新table，然后到新table中重新get
				return get_impl(topmap,
						chm.copy_slot_and_check(topmap, kvs, idx, key), key,
						fullhash); // Retry in the new table
			}
			// get and put must have the same key lookup logic! But only 'put'
			// needs to force a table-resize for a too-long key-reprobe
			// sequence.
			...
			if (++reprobe_cnt >= reprobe_limit(len) || // too many probes
					K == TOMBSTONE) // found a TOMBSTONE key, means no more keys
									// in this table
				//如果碰撞次数太多或者发现一个TOMBSTONE key，则会触发rehash
				return newkvs == null ? null : get_impl(topmap,
						topmap.help_copy(newkvs), key, fullhash); // Retry in
																	// the new
																	// table
	...
	private static final Object putIfMatch(final NonBlockingHashMap topmap,
			final Object[] kvs, final Object key, final Object putval,
			final Object expVal) {
		assert putval != null;
		assert !(putval instanceof Prime);
		assert !(expVal instanceof Prime);
		final int fullhash = hash(key); // throws NullPointerException if key
										// null
		final int len = len(kvs); // Count of key/value pairs, reads kvs.length
		...
		while (true) { // Spin till we get a Key slot
			...
			if (K == null) { // Slot is free?
				// Found an empty Key slot - which means this Key has never been
				// in
				// this table. No need to put a Tombstone - the Key is not here!
				//这里putval为Tombstone表示key必须已经存在
				//因为k==null表示key不存在，所以可以直接返回
				if (putval == TOMBSTONE)
					return putval; // Not-now & never-been in this table
				
				// Claim the null key-slot
				// CAS方式写入key，并更新slots和hashes
				...
				// of Java
				// CAS. Most hardware CAS's report back the existing value - so
				// that
				// if you fail you have a *witness* - the value which caused the
				// CAS
				// to fail. The Java API turns this into a boolean destroying
				// the
				// witness. Re-reading does not recover the witness because
				// another
				// thread can write over the memory after the CAS. Hence we can
				// be in
				// the unfortunate situation of having a CAS fail *for cause*
				// but
				// having that cause removed by a later store. This turns a
				// non-spurious-failure CAS (such as Azul has) into one that can
				// apparently spuriously fail - and we avoid apparent spurious
				// failure
				// by not allowing Keys to ever change.
				K = key(kvs, idx); // CAS failed, get updated value 没有使用volatile,如何保证能“看到”新值？
				assert K != null; // If keys[idx] is null, CAS shoulda worked  因为删除一个key-value会放入一个TOMBSTONE，所以1个key的位置一旦被放入过key-value，就不可能再为空
			}
			...
			if (++reprobe_cnt >= reprobe_limit(len) || // too many probes or
					K == TOMBSTONE) { // found a TOMBSTONE key, means no more
										// keys
				// We simply must have a new table to do a 'put'. At this point
				// a
				// 'get' will also go to the new table (if any). We do not need
				// to claim a key slot (indeed, we cannot find a free one to
				// claim!).
		...
		if (newkvs == null && // New table-copy already spotted?
				...
				// Or we found a Prime, but the JMM allowed reordering such that
				// we
				// did not spot the new table (very rare race here: the writing
				// thread did a CAS of _newkvs then a store of a Prime. This
				// thread
				// reads the Prime, then reads _newkvs - but the read of Prime
				// was so
				// delayed (or the read of _newkvs was so accelerated) that they
				// swapped and we still read a null _newkvs. The resize call
				// below
				// will do a CAS on _newkvs forcing the read.
				V instanceof Prime))
			newkvs = chm.resize(topmap, kvs); // Force the new table copy to
												// start
		// See if we are moving to a new table.
		// If so, copy our slot and retry in the new table.
		if (newkvs != null)
			return putIfMatch(topmap,
					chm.copy_slot_and_check(topmap, kvs, idx, expVal), key,
					putval, expVal);
		...
		while (true) {
			...
			// Must match old, and we do not? Then bail out now. Note that
			// either V
			...
			if (expVal != NO_MATCH_OLD
					...
					&& !(V == null && expVal == TOMBSTONE) && // Match on
																// null/TOMBSTONE
																// combo
					(expVal == null || !expVal.equals(V))) // Expensive equals
															// check at the last
			...
			// If a Prime'd value got installed, we need to re-run the put on
			// the
	...
	private static final class CHM<TypeK, TypeV> {
		...
		// ---
		// These next 2 fields are used in the resizing heuristics, to judge
		// when
		// it is time to resize or copy the table. Slots is a count of used-up
		// key slots, and when it nears a large fraction of the table we
		// probably
		...
		// Count of used slots, to tell when table is full of dead unusable
		// slots
		...
		// volatile variable that is read as we cross from one table to the
		// next,
		...
		private static final AtomicReferenceFieldUpdater<CHM, Object[]> _newkvsUpdater = AtomicReferenceFieldUpdater
				.newUpdater(CHM.class, Object[].class, "_newkvs");
		...
		// give the initial resizing thread a little time to allocate the
		// initial
		// new table. The Right Long Term Fix here is to use array-lets and
		// incrementally create the new very large array. In C I'd make the
		// array
		...
		volatile long _resizers; // count of threads attempting an initial
									// resize
		private static final AtomicLongFieldUpdater<CHM> _resizerUpdater = AtomicLongFieldUpdater
				.newUpdater(CHM.class, "_resizers");
		...
		// high and we must report the table is full. If we do not, then we
		// might
		// end up deciding that the table is not full and inserting into the
		// current table, while a 'get' has decided the same key cannot be in
		// this
		...
		// Callers will (not this routine) will 'help_copy' any in-progress
		// copy.
		// Since this routine has a fast cutout for copy-already-started,
		// callers
		...
		private final Object[] resize(NonBlockingHashMap topmap, Object[] kvs) {
			...
			// Check for resize already in progress, probably triggered by
			// another thread
			...
			// No copy in-progress, so start one. First up: compute new table
			// size.
			...
			// Heuristic to determine new size. We expect plenty of
			// dead-slots-with-keys
			...
			// down resize operations for tables subject to a high key churn
			// rate.
			long tm = System.currentTimeMillis();
			long q = 0;
			if (newsz <= oldlen && // New table would shrink or hold steady?
					tm <= topmap._last_resize_milli + 10000 && // Recent resize
																// (less than 1
																// sec ago)
					(q = _slots.estimate_get()) >= (sz << 1)) // 1/2 of keys are
																// dead?
			...
			// We
			// guess at 32-bit pointers; 64-bit pointers screws up the size calc
			// by
			// 2x but does not screw up the heuristic very much.
			int megs = ((((1 << log2) << 1) + 4) << 3/* word to bytes */) >> 20/* megs */;
			if (r >= 2 && megs > 0) { // Already 2 guys trying; wait and see
				newkvs = _newkvs; // Between dorking around, another thread did
									// it
				if (newkvs != null) // See if resize is already in progress
					return newkvs; // Use the new table already
				// TODO - use a wait with timeout, so we'll wakeup as soon as
				// the new table
				// is ready, or after the timeout in any case.
				// synchronized( this ) { wait(8*megs); } // Timeout - we always
				// wakeup
				// For now, sleep a tad and see if the 2 guys already trying to
				// make
			...
			// Last check, since the 'new' below is expensive and there is a
			// chance
			// that another thread slipped in a new thread while we ran the
			// heuristic.
			newkvs = _newkvs;
			if (newkvs != null) // See if resize is already in progress
				return newkvs; // Use the new table already
			...
			newkvs = new Object[((1 << log2) << 1) + 2]; // This can get
															// expensive for big
															// arrays
		...
		// The next part of the table to copy. It monotonically transits from
		// zero
		// to _kvs.length. Visitors to the table can claim 'work chunks' by
		// CAS'ing this field up, then copying the indicated indices from the
		// old
		// table to the new table. Workers are not required to finish any chunk;
		// the counter simply wraps and work is copied duplicately until
		// somebody
		// somewhere completes the count.
		volatile long _copyIdx = 0;
		static private final AtomicLongFieldUpdater<CHM> _copyIdxUpdater = AtomicLongFieldUpdater
				.newUpdater(CHM.class, "_copyIdx");

		// Work-done reporting. Used to efficiently signal when we can move to
		// the new table. From 0 to len(oldkvs) refers to copying from the old
		// table to the new.
		volatile long _copyDone = 0;
		static private final AtomicLongFieldUpdater<CHM> _copyDoneUpdater = AtomicLongFieldUpdater
				.newUpdater(CHM.class, "_copyDone");

		// --- help_copy_impl
		// ----------------------------------------------------
		// Help along an existing resize operation. We hope its the top-level
		// copy (it was when we started) but this CHM might have been promoted
		// out
		// of the top position.
		private final void help_copy_impl(NonBlockingHashMap topmap,
				Object[] oldkvs, boolean copy_all) {
			...
			final int MIN_COPY_WORK = Math.min(oldlen, 1024); // Limit
																// per-thread
																// work
			...
			while (_copyDone < oldlen) { // Still needing to copy?
				...
				// We "panic" if we have tried TWICE to copy every slot - and it
				// still
				// has not happened. i.e., twice some thread somewhere claimed
				// they
				// would copy 'slot X' (by bumping _copyIdx) but they never
				// claimed to
				// have finished (by bumping _copyDone). Our choices become
				// limited:
				// we can wait for the work-claimers to finish (and become a
				// blocking
				// algorithm) or do the copy work ourselves. Tiny tables with
				// huge
				// thread counts trying to copy the table often 'panic'.
				if (panic_start == -1) { // No panic?
					copyidx = (int) _copyIdx;
					while (copyidx < (oldlen << 1) && // 'panic' check
							!_copyIdxUpdater.compareAndSet(this, copyidx,
									copyidx + MIN_COPY_WORK))
						copyidx = (int) _copyIdx; // Re-read
					if (!(copyidx < (oldlen << 1))) // Panic!
						panic_start = copyidx; // Record where we started to
												// panic-copy
				}
				...
				for (int i = 0; i < MIN_COPY_WORK; i++)
					if (copy_slot(topmap, (copyidx + i) & (oldlen - 1), oldkvs,
							newkvs)) // Made an oldtable slot go dead?
						workdone++; // Yes!
				if (workdone > 0) // Report work-done occasionally
					copy_check_and_promote(topmap, oldkvs, workdone);// See if
																		// we
																		// can
																		// promote
				// for( int i=0; i<MIN_COPY_WORK; i++ )
				...
				// copy_check_and_promote( topmap, oldkvs, 1 );// See if we can
				// promote

				copyidx += MIN_COPY_WORK;
				// Uncomment these next 2 lines to turn on incremental
				// table-copy.
			...
			// Extra promotion check, in case another thread finished all
			// copying
		...
		// read it yet.
		private final Object[] copy_slot_and_check(NonBlockingHashMap topmap,
				Object[] oldkvs, int idx, Object should_help) {
			assert chm(oldkvs) == this;
			Object[] newkvs = _newkvs; // VOLATILE READ
			// We're only here because the caller saw a Prime, which implies a
			// table-copy is in progress.
			assert newkvs != null;
			if (copy_slot(topmap, idx, oldkvs, _newkvs)) // Copy the desired
															// slot
				copy_check_and_promote(topmap, oldkvs, 1); // Record the slot
															// copied
			// Generically help along any copy (except if called recursively
			// from a helper)
		...
		private final void copy_check_and_promote(NonBlockingHashMap topmap,
			...
			if (workdone > 0) {
				while (!_copyDoneUpdater.compareAndSet(this, copyDone, copyDone
						+ workdone)) {
			...
			// Check for copy being ALL done, and promote. Note that we might
			// have
			// nested in-progress copies and manage to finish a nested copy
			// before
			...
			if (copyDone + workdone == oldlen && // Ready to promote this table?
				...
				topmap._last_resize_milli = System.currentTimeMillis(); // Record
																		// resize
																		// time
																		// for
																		// next
		...
		// slot. We need an accurate confirmed-copy count so that we know when
		// we
		...
		// any direct updates on the new table, unless they first happened to
		// the
		// old table - so that any transition in the new table from null to
		// not-null must have been from a copy_slot (or other old-table
		// overwrite)
		// and not from a thread directly writing in the new table. Thus we can
		// count null-to-not-null transitions in the new table.
		private boolean copy_slot(NonBlockingHashMap topmap, int idx,
				Object[] oldkvs, Object[] newkvs) {
			// Blindly set the key slot from null to TOMBSTONE, to eagerly stop
			// fresh put's from inserting new values in the old table when the
			// old
			// table is mid-resize. We don't need to act on the results here,
			// because our correctness stems from box'ing the Value field.
			// Slamming
			...
			while (!(oldval instanceof Prime)) {
				final Prime box = (oldval == null || oldval == TOMBSTONE) ? TOMBPRIME
						: new Prime(oldval);
				if (CAS_val(oldkvs, idx, oldval, box)) { // CAS down a box'd
															// version of oldval
			...
			// ---
			// Copy the value into the new table, but only if we overwrite a
			// null.
			// If another value is already in the new table, then somebody else
			// wrote something there and that write is happens-after any value
			// that
			// appears in the old table. If putIfMatch does not find a null in
			// the
			...
			boolean copied_into_new = (putIfMatch(topmap, newkvs, key,
					old_unboxed, null) == null);

			// ---
			// Finally, now that any old value is exposed in the new table, we
			// can
			// forever hide the old-table value by slapping a TOMBPRIME down.
			// This
			// will stop other threads from uselessly attempting to copy this
			// slot
	...
	private class SnapshotV implements Iterator<TypeV>, Enumeration<TypeV> {
		...
		public SnapshotV() {
			while (true) { // Verify no table-copy-in-progress
				...
				if (topchm._newkvs == null) { // No table-copy-in-progress
					// The "linearization point" for the iteration. Every key in
					// this
					// table will be visited, but keys added later might be
					// skipped or
					// even be added to a following table (also not iterated
					// over).
					_sskvs = topkvs;
					break;
				}
				// Table copy in-progress - so we cannot get a clean iteration.
				// We
				// must help finish the table copy before we can start
				// iterating.
		...
		public TypeV next() {
			...
			while (_idx < length()) { // Scan array
				_nextK = key(_idx++); // Get a key that definitely is in the set
										// (for the moment!)
		...
		public void remove() {
			...
			putIfMatch(NonBlockingHashMap.this, _sskvs, _prevK, TOMBSTONE,
					_prevV);
			_prevV = null;
		}
	 ...
	 * 
	 * <p>
	 * <strong>Warning:</strong> the iterator associated with this Set requires
	 * the creation of {@link java.util.Map.Entry} objects with each iteration.
	 * The {@link NonBlockingHashMap} does not normally create or using
	 * {@link java.util.Map.Entry} objects so they will be created soley to
	 * support this iteration. Iterating using {@link #keySet} or
	 * {@link #values} will be more efficient.
	 */
	@Override
